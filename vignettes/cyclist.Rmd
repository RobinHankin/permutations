...
This vignette discusses the concept of cyclists as used in the
permutations package.

A "cyclist" is an object corresponding to a permutation P.  It is a
list with elements that are integer vectors corresponding to the
cycles of P.  This object is informally known as a cyclist, but there
is no S3 class corresponding to it.  For example
...
whose elements are permutations written in cycle form.

The elements of a cyclist are the disjoint group-theoretic cycles.  A
cyclist may be poorly formed in a number of ways: the cycles may
...
A cyclist nevertheless possesses numerous redundancies: firstly,
...
the smallest (these are not serious in the sense that their presense
...
(the number 3 is repeated).  Function `char2cyuclist_single()` does
