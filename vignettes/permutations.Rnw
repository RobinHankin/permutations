...
\Abstract{
  ...
  manipulating and permutations of a finite set.  I show how the
...
extended discusssion is given in \code{cyclist.Rd}.
...
syntatic sugar of the extraction operator~\code{[()}.  An extended
...
documentaition is a at \code{shape.Rd} and \code{conjugate.Rd}.
...
One particularly pleasing pattern on the megaminx is the
...
(note the use of binary ``+'' showing that sequences
...
\begin{figure}[h]
  \centering \includegraphics{megaminx_net.pdf}
