...
automorphism].  Here, the general plan will be as follows.  Firstly,
express any element of $S_6$ as a product of cycles of the form $(1n)$
for $2\leqslant n\leqslant 6$; secondly, apply $\phi$ as defined above
and, assuming $\phi$ to be a homomorphism, exhibit an explicit outer
autormorphism of $S_6$

First we define `f()` that gives transpositions of the form $(1n)$:
...
It doesn't look like it has worked, but it has:
...
each element with each other element.  This takes about 10 minutes to
run, too long for a package vignette.

```{r checkphiisomorphism,eval=FALSE}
M <- matrix(0,720,720)
for(i in 1:720){for(j in 1:720){M[i,j] <- x[i] == x[j]}}
all(M == diag(720))  # evaluates to TRUE
```
