...
```a``` but in an obscure form.  To answer the question "who is
sitting in place $i$?" we need to *invert* permutation ```a```:
...
and the R object would use the inverse of ```a```:
...
Now we consider the same operations ```a``` and ```b``` but from the
perspective of places, rather than people.
